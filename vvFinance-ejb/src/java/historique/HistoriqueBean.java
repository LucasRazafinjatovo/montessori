...
@Stateful
@AccessTimeout(0)
public class HistoriqueBean implements Historique, HistoriqueRemote, SessionBean {
  ...
  public String createUtilisateurs(String loginuser, String pwduser, String nomuser, String adruser, String teluser, String idrole, String refUser) throws Exception {
    ...
    try {
      ...
      if(uT.length>0)    throw new Exception("Login déjà utilisé!");

      String passCrypt = null;
      int niveau = (int)Math.round(Math.random()*10);
      int sens = (int)Math.round(Math.random());
      if(niveau == 0) niveau = -5; //raha zero ny niveau de atao -5
      if (sens == 0)  passCrypt = Utilitaire.cryptWord(pwduser.toLowerCase(), niveau, true);
      else  passCrypt = Utilitaire.cryptWord(pwduser.toLowerCase(), niveau, false);
...
//    MapUtilisateur u=new MapUtilisateur(utilitaire.Utilitaire.stringToInt(iduser),loginuser, pwduser, nomuser, adruser, teluser, idrole);
  ...
  public MapUtilisateur[] findUtilisateurs(String refuser, String loginuser, String pwduser, String nomuser, String adruser, String teluser, String idrole) throws Exception {
    try {
      int[] a={1,2,3,4,5,6,7}; //Donne le numéro des champs sur lesquelles on va mettre des critères
      String[] val=new String[a.length];
      val[0]=refuser;val[1]=loginuser;val[2]=pwduser;val[3]=nomuser;val[4]=adruser;val[5]=teluser;val[6]=idrole; //Affecte des valeurs aux critères
