...
<%
try{
p=new MvtCaisseTvaHT();
p.setNomTable("MVTCAISSETVAHT");
...
String libEntete[]={"id","daty","designation","agence","idCaisse","debit","credit", "TVA", "HT"};
PageRecherche pr=new PageRecherche(p,request,listeCrt,listeInt,2,libEntete,9);

pr.setUtilisateur((user.UserEJB)session.getValue("u"));
pr.setLien((String)session.getValue("lien"));
pr.setApres("finance/listeMvtCaissetvaetht.jsp");
...
String colSomme[]={"tva", "ht", "debit", "credit"};
pr.creerObjetPageMultiple(libEntete,colSomme);
//pr.preparerDataFormu();
%>
<h1>Lister les Mouvements de caisse sans Ligne credit</h1>
<form action="<%=pr.getLien()%>?but=finance/listeMvtCaissetvaetht.jsp" method="post" name="listePub" id="listePub">
<%
