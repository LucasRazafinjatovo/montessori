...
for (int i=0;i<idLcArecup.length;i++)
{
  ...
  if(acte.compareToIgnoreCase("update")==0) u.updateMontantUnionIntra(Constante.tableOpFfLc,numObjet,idLcArecup,montant);
  if(acte.compareToIgnoreCase("delete")==0) u.deleteUnionIntraTable(Constante.tableOpFfLc,idUnion);
  if(acte.compareToIgnoreCase("svalide")==0)  u.validerMappageLCMOp("Opvisefflc",numObjet);
 %>
<script language="JavaScript"> document.location.replace("<%=lien %>?but=ded/ventilationOpFf.jsp&numObjet=<%=numObjet%>"); </script>
<%
}
catch(Exception e){
  e.printStackTrace();
