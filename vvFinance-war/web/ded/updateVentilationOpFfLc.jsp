...
for (int i=0;i<idLcArecup.length;i++)
{
  ...
  if(acte.compareToIgnoreCase("update")==0) u.updateMontantUnionIntraLcOp(Constante.tableOpFfLc,"FACTUREFOURNISSEURLC",numObjet,idLcArecup,montant);
  if(acte.compareToIgnoreCase("delete")==0) u.deleteUnionIntraTableLcOp(Constante.tableOpFfLc,"FACTUREFOURNISSEURLC",idUnion);
  if(acte.compareToIgnoreCase("svalide")==0)  u.validerMappageLCMOpControle(Constante.tableOpFfLc,"FACTUREFOURNISSEURLC",numObjet);
 %>
<script language="JavaScript"> document.location.replace("<%=lien %>?but=ded/ventilationOpFfLc.jsp&numObjet=<%=numObjet%>"); </script>
<%
}
catch(Exception e){
  e.printStackTrace();
