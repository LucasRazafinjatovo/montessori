...
try{
        ...
        System.out.println("Taille op "+op.length);
        lcc=(UnionIntraTable[])uit.rechercher(2,numObjet);
        lccAchoix=u.findFCC(idLC,designation,mois1,mois2,idType,numCompte,"%","%","%",entite,direction,colonne,ordre, annee,"lignecreditcomplet");
...
<form action="<%=lien%>?but=ded/updateVentilationOpFfLc.jsp" method="POST" name="frmListe" id="frmListe">
  <p align="center"><strong><u>LISTE SELECTION </u></strong></p>
  <p align="center"><u><strong><a title="montant op="<%=op[0].getMontant()%> href=<%=lien%>?but=ded/visualiserOP.jsp&idOP=<%=numObjet%>>Voir d&eacute;tails op num <%=numObjet%></a></strong></u></p>
  ...
  %>
    <tr>
      ...
      <td align="right" valign="top"><div align="center" class="Style1"><a href="<%=lien %>?but=ded/updateVentilationOpFfLc.jsp&idUnion=<%=lcc[i].getId()%>&acte=delete&numObjet=<%=lcc[i].getId1()%>">X</a></div></td>
    </tr>
    <%
  }
  %>
...
<form action="<%=lien %>?but=ded/ventilationOpFfLc.jsp&numObjet=<%=numObjet%>" method="post" name="listeLC" id="listeLC">
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
 ...
 <br />
   ...
   <form action="<%=lien%>?but=lc/saveOpFfLcJointure.jsp" method="POST" name="frmChoix" id="frmChoix">
