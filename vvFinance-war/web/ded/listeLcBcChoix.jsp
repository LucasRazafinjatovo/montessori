...
String indiceSelection=request.getParameter("indice");
  ...
  montantObjet=request.getParameter("montantObjet"+indiceSelection);if ((montantObjet==null)||montantObjet.compareTo("")==0) montantObjet="0";
                ...
                //factF = u.findFactureF("%", "", "", "%", "%","%", "idFactureFournisseur", "ASC", "");
                        bc = u.findEntFact("BC",id, numBC, designation, fournisseur,"%","%","%",dateDebut,dateFin,"%");
                                  int pejy=0;
          if(pazy==0){
                        pejy=0;
                        pazy=1;
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
  <tr>
    <td> <p align="center"><strong><u>RECAPITULATION RECHERCHE</u></strong></p>
      ...
      <br>
      <p align="center"><strong><u>LISTE</u></strong></p><form action="<%=lien%>?but=ded/apresChoixBc.jsp" method="post">
...
<td align="center"><input name="idBC" class="radio"  type='checkbox' value="<%=bc[i].getTuppleID()%>"></td>
...
<%if(pazy!=1){%>
        <a href="<%=lien%>?but=ded/listeLcBcChoix.jsp&pazy=<%=pazy-1%>&numero=<%=Utilitaire.remplacePourcentage(numero)%>&fournisseur=<%=Utilitaire.remplacePourcentage(fournisseur)%>&dateDebut=<%=dateDebut%>&dateFin=<%=dateFin%>&projet=<%=Utilitaire.remplacePourcentage(projet)%>&designation=<%=Utilitaire.remplacePourcentage(designation)%>&colonne=<%=colonne%>&numObjet=<%=numObjet%>&typeObjet=<%=typeObjet%>&montantObjet=<%=montantObjet%>&ordre=<%=ordre%>">&lt;&lt;Page
...
<script language="javascript">
  ...
  var cal3 = new calendar1(document.forms['listeRecette'].elements['dateEmDebut']);
  cal3.year_scroll = false;
  cal3.time_comp = false;
  var cal4 = new calendar1(document.forms['listeRecette'].elements['dateEmFin']);
