...
String etat = "PREVDEPENSEVISENONOP";
e.setNomTable(etat);
...
pr.setApres("prevision/listePrevisionDepNonPaye.jsp");
...
<form action="<%=pr.getLien()%>?but=prevision/listePrevisionDepNonPaye.jsp" method="post" name="listePrevisionDetail" id="listePrevisionDetail">
<%
...
<td align=center><a href="<%=pr.getLien()%>?but=prevision/fichePrevisionDetail.jsp&id=<%=listeP[i].getId()%>"><%=listeP[i].getId() %></a></td>
...
<td align=center><a href="<%=pr.getLien()+"?but=prevision/apresPrevision.jsp&acte=delete&numObjet="+listeP[i].getId()+"&crt="+pr.getFormu().getListeCritereString()+"&numPage="+pr.getNumPage()+"&classe=PrevisionDepense"%>">X</a></td>
</tr>
<input type="hidden" name="id<%=i%>" value="<%=listeP[i].getId()%>">
<%}%>
</table>
<input type="hidden" name="nb" value="<%=listeP.length%>">
<input type="hidden" name="bute" value="prevision/listePrevisionDepNonPaye.jsp">
<input type="hidden" name="classe" value="PrevisionDepense">
