...
for (int i=0;i<idLcArecup.length;i++)
{
 ...
 //public String updateMontantUnionIntraLcOp(String nomTable, String nomTable2, String id1, String[] id2, String[] montant) throws Exception {
  ...
  u.updateMontantUnionIntraLcPrev("PREVRECETTELC",numObjet,idLcArecup,montant);
  }
  ...
  {
    u.validerMappageLCPrevRecControle("PREVRECETTELC",numObjet);
  }
 %>
<script language="JavaScript"> document.location.replace("<%=lien %>?but=prevision/ventilationPrevLcRecette.jsp&numObjet=<%=numObjet%>&typeor=<%=request.getParameter("typeor")%>"); </script>
<%
}
catch(Exception e){
  e.printStackTrace();
