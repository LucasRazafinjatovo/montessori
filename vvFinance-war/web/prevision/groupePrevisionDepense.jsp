...
p.setNomTable("PREVDEPENSEVISELC");
...
PageRechercheGroupe pr=new PageRechercheGroupe(p,request,listeCrt,listeInt,3,colDefaut,somDefaut,4,1);

pr.setUtilisateur((user.UserEJB)session.getValue("u"));
pr.setLien((String)session.getValue("lien"));
pr.setApres("prevision/groupePrevisionDepense.jsp");
...
pr.creerObjetPage();
%>
<h1>Grouper pr&eacute;vision depense</h1>
<form action="<%=pr.getLien()%>?but=prevision/groupePrevisionDepense.jsp" method="post" name="groupePrev" id="groupePrev">
<%
...
<%
String lienTableau[]={pr.getLien()+"?but=prevision/fichePrevisionDetail.jsp"+pr.getFormu().getListeCritereString(),pr.getLien()+"?but=lc/visualiserDetailLC.jsp"+pr.getFormu().getListeCritereString()};
