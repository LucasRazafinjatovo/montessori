...
<h1>Tableau r&eacute;capitulatif de pr&eacute;vision</h1>
<form action="<%=(String)session.getValue("lien")%>?but=prevision/listePrevisionEtat.jsp" method="post" name="listePrevisionEtat" id="listePrevisionEtat">
				...
				for(int i = 0; i < listEtatPrev.length; i++){
					...
					<tr onmouseover="this.style.backgroundColor='#EAEAEA'" onmouseout="this.style.backgroundColor=''">
						...
						<td width="25%" align="right">
							<a href="<%=(String)session.getValue("lien")%>?but=prevision/detailsPrevisionOP.jsp&daty=<%=Utilitaire.convertDatyFormtoRealDatyFormat(listEtatPrev[i].getDaty()+"")%>" target="_blank"><%=Utilitaire.formaterAr(listEtatPrev[i].getDebit())%></a>
						</td>
						<td width="25%" align="right">
							<a href="<%=(String)session.getValue("lien")%>?but=prevision/detailsPrevisionOR.jsp&daty=<%=Utilitaire.convertDatyFormtoRealDatyFormat(listEtatPrev[i].getDaty()+"")%>" target="_blank"><%=Utilitaire.formaterAr(listEtatPrev[i].getCredit())%></a>
						</td>
