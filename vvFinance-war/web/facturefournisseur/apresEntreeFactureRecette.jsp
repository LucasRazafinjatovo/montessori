...
out.println("acte = "+acte);
	try
	{
        ...
        if (acte.compareTo("INSERT")==0)
        {
			...
			%>
			<script language="JavaScript"> document.location.replace("<%=lien%>?but=facturefournisseur/entreeFacture.jsp&nature=<%=request.getParameter("nature")%>");
			</script>
			<%
        }
        if (acte.compareTo("VISER")==0)
        {
			rep = u.viserFactureC(request.getParameter("numFact"), request.getParameter("remarque"));
...
%>
          ...
          if (acte.compareTo("VISERFACTPROF")==0)
          {
                        rep=u.viserFactureCProf(request.getParameter("numFact"), request.getParameter("remarque"));

%>
          <script language="JavaScript"> document.location.replace("<%=lien%>?but=facture/factureInitProf.jsp");
                  </script>
          <%
          }
...
%>
        ...
        <%
          ...
          {
           %>
 		 <script language="JavaScript"> document.location.replace("<%=lien%>?but=lc/apresChoixListeLCRecette.jsp&numObjet=<%=request.getParameter("numFact")%>"); </script>
        <%
          }
          if (acte.compareTo("UPDATE")==0)
          {
        ...
        //methode update
           ...
           %>
 		 <script language="JavaScript"> //document.location.replace("<%=lien%>?but=facture/critereEntree.jsp"); </script>
        <%
          }
