...
if(datyInf>datySup){
alert('Date inférieur doit être inférieur');
 return false;
}else{
 alert('kozy');
 return false;
  ...
  for (i=0; i<(args.length-2); i+=3) { test=args[i+2]; val=MM_findObj(args[i]);
    if (val) { nm=val.name; if ((val=val.value)!="") {
      if (test.indexOf('isEmail')!=-1) { p=val.indexOf('@');
        ...
        if (isNaN(val)) errors+='- '+nm+' doit être de type numérique.\n';
        if (test.indexOf('inRange') != -1) { p=test.indexOf(':');
          min=test.substring(8,p); max=test.substring(p+1);
          if (num<min || max<num) errors+='- '+nm+' must contain a number between '+min+' and '+max+'.\n';
    } } } else if (test.charAt(0) == 'R') errors += '- '+nm+' ne doit pas être vide.\n'; }
  } if (errors) alert('Des erreurs ont été rencontrée lors de votre saisie. Veuillez vérifier les valeurs entrées!\n');
  //alert('Des erreurs ont été rencontré lors de votre saisie :\n'+errors);
...
/****************************************/
 ...
 function datecomp(datesaisie,datecompar){
    ...
    if (datefin=='1') {
       ...
       fin[3]=datef.getYear();
       var message="La date début doit être inférieure à la date de fin";
    }
...
function pagePopUp(page){
     window.open(page,"","titulaireresizable=no,scrollbars=yes,location=no,width=710,height=580,top=0,left=0");
 }