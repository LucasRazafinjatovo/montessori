...
cltCrt.setNomTable("comptecomptatiers");
...
<form action="choixCompteComptaTiers.jsp" method="post">
...
<td  width="35%"  align="left"><input name="desce" id="desce" type="text" value="<%=desce%>" size="25" style="text-align:left;"></td>
<td></td>
</tr>
</table>
...
<input type="hidden" name="champReturn" value="<%=champReturn%>">
                <table align="center" class="monographe" width="100%">
					...
					<tr>
						<td><input type="radio" name="choix" onMouseDown="getChoix()" value="<%=clt[i].getVal()+";"+clt[i].getDesce()%>" class="radio" /></td>
						<td><%=clt[i].getId()%> </td>
                        <td><%=clt[i].getVal()%>  </td>
						<td><%=clt[i].getDesce()%></td>
          </tr>
<%
...
<%if(pazy!=1){%>
         <a href="choixCompteComptaTiers.jsp?id=<%=Utilitaire.remplacePourcentage(id)%>&val=<%=Utilitaire.remplacePourcentage(val)%>&desce=<%=Utilitaire.remplacePourcentage(desce)%>&pazy=<%=pazy-1%>">&lt;&lt;Page
