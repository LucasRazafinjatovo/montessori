...
try{
        ...
        //bc=u.findFactureFournisseur("FACTUREFOURNISSEURLETTRE", numObjet, "", "", "%","%","daty","asc", "");
        if(bc.length==0) throw new Exception("Facture fournisseur non existante");
...
document.location.replace("<%=lien%>?but=erreur.jsp&message=<%=e.getMessage()%>");
</script>
<%
  return;
}
...
<script type="text/javascript">
 ...
 document.liste.submit();
  ...
  function calculPourcentage(i)
{
alert(frmListe.montant0.value);
}
...
<form action="<%=lien %>?but=lc/apresChoixListeLC.jsp&numObjet=<%=numObjet%>" method="post" name="listeLC" id="listeLC">
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
 ...
 <br />
   ...
   <form action="<%=lien%>?but=lc/saveLCJointure.jsp" method="POST" name="liste">
