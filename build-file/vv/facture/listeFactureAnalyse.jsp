    ...
    <tr>
      <td> <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="monographe">
          ...
          <tr>
            ...
            <td><input name="numFact" type="text" class="champ" id="numFact" size="10" value="<%=numFact%>"></td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td height="30" align="center">
	   <table width="75%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td align="center"> <input name="Annuler" type="reset" class="submit" value="Annuler">
            </td>
          </tr>
        </table>
       </td>
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
  ...
  <tr>
    <td> <p align="center"><strong><u>RECAPITULATION</u></strong></p>
      ...
      %>
        <tr class="head">
          <td  height="25" align="center" bgcolor="#C300FF"><strong><%=cat[k].getVal()%></strong> </td>
        ...
        <tr>
          <td width="69"  height="25" align="center"><strong>Total <%=cat[k].getVal()%> :</strong></td>
          <td width="131" align="right"><%=Utilitaire.formaterAr((double)AdminFacture.calculSommeHTvaFacture(fm,cat[k].getId()))%> <%=" "+cat[k].getId()%></td>
          <td width="140" align="center"><div align="right"><%=Utilitaire.formaterAr((double)AdminFacture.calculSommeTvaFacture(fm,cat[k].getId()))%><%=" "+cat[k].getId()%></div></td>
          <td width="135" align="center"><div align="right"><%=Utilitaire.formaterAr((double)AdminFacture.calculSommeEtatFacture(fm,cat[k].getId()))%><%=" "+cat[k].getId()%></div></td>
          <td width="81" align="left"><div align="right">&nbsp;<strong> Nbre</strong>
              :<%=facture.AdminFacture.getNombreFacture(fm,cat[k].getId())%> </div></td>
          <td width="40" align="left"><div align="center"><a href="<%=lien%>?but=facture/detailListeFactureAnalyse.jsp&datyInf=<%=datyinf%>&datySup=<%=datySup%>&client=<%=Utilitaire.remplacePourcentage(client)%>&etat=<%=idEtat%>&idCat=<%=cat[k].getId()%>&sscat=<%=idSsCatServ%>&numFact=<%=Utilitaire.remplacePourcentage(numFact)%>">D&eacute;tails</a></div></td>
        </tr>
        <%
         }
      }%>
