...
//listepub = request.getParameter("idpub");
listepub = (String)session.getValue("idpublicite");
session.removeAttribute("idpublicite");
%>
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="monographe">
        ...
        <%if(acte.compareToIgnoreCase("invendu")==0)
        {
        %>
                 <td valign="top">&nbsp; <input name="idTva" type="text" class="champ" maxlength="3" value="0"<%if(ut.getIdrole().compareTo("compta")==0 || ut.getIdrole().compareTo("controle")==0 || ut.getIdrole().compareTo("pilotage")==0|| ut.getIdrole().compareTo("audit")==0) out.print(" disabled");%>>%
        <%
        } else if(acte.compareToIgnoreCase("pub")==0){
        %>
                   <input type="hidden" name="datyparu" value="<%=request.getParameter("datyparu")%>">
                   <input type="hidden" name="ttva" value="<%=request.getParameter("ttva")%>">
                        <%
                        if(request.getParameter("ttva").compareToIgnoreCase("ttva1")==0){
                        %>
                        <td valign="top">&nbsp; <input name="idTva" type="text" class="champ" maxlength="3" readonly value="20"<%if(ut.getIdrole().compareTo("controle")==0 || ut.getIdrole().compareTo("pilotage")==0|| ut.getIdrole().compareTo("audit")==0) out.print(" disabled");%>>%
                        <%}
                   else{
                   %>
                         <td valign="top">&nbsp; <input name="idTva" type="text" class="champ" maxlength="3" readonly value="0"<%if(ut.getIdrole().compareTo("controle")==0 || ut.getIdrole().compareTo("pilotage")==0|| ut.getIdrole().compareTo("audit")==0) out.print(" disabled");%>>%
                        <%
                        }
           }else{%>
         <td valign="top">&nbsp; <input name="idTva" type="text" class="champ" maxlength="3" value="<%=tva.getValTva()%>"<%if(ut.getIdrole().compareTo("controle")==0 || ut.getIdrole().compareTo("pilotage")==0|| ut.getIdrole().compareTo("audit")==0) out.print(" disabled");%>>%
          <%}%> <input name="acte" type="hidden" id="acte2" value="INSERT">
          <input name="fact" type="hidden" id="acte" value="<%=acte%>"></td>
