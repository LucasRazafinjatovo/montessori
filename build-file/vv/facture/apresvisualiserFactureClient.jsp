...
<%}
  if(acte.compareToIgnoreCase("attP")==0){
  %>

                 <script language="JavaScript"> document.location.replace("<%=lien%>?but=pub/listePubFacture.jsp&idFacture=<%=numObjet%>"); </script>


<%}
  if(acte.compareToIgnoreCase("attDist")==0){
    %>

                   <script language="JavaScript"> document.location.replace("<%=lien%>?but=ventegazety/listeDistFacture.jsp&idFacture=<%=numObjet%>"); </script>
...
%>
	       ...
	       <script language="JavaScript"> document.location.replace("<%=lien%>?but=lc/apresChoixListeLCRecette.jsp&idLC=&numObjet=<%=numObjet%>&typeObjet=<%=typeObjet%>&montantObjet=<%=montantObjet%>"); </script>
...
<%}
 if(acte.compareToIgnoreCase("modifier")==0){
	 %>
	 	       ...
	 	       <script language="JavaScript"> document.location.replace("<%=lien%>?but=facturefournisseur/modifierGen.jsp&numObjet=<%=numObjet%>&nature=<%=Constante.objetFactureClient %>&tableDet=<%=Constante.getObjectDetailFPF()%>"); </script>
...
//annulerOp(String nomTable, String nomTableLiaison, String id)
           u.annulerFact("FACTURECLIENT","FACTURECLIENTLC",numObjet);
           %>
           }
           document.location.replace("<%=lien%>?but=facture/visualiserFactureClient.jsp&idBC=<%=numObjet%>");
