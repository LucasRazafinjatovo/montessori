...
typeE = u.findTypeObjet("typeEntree","%","%");
...
sc=u.findSyntheseCompteEntreeType(dateDebut, dateFin, caisse, entite, catCompte, cat,idT,va);
...
<h1>Synth&egrave;se des recettes selon cat&eacute;gories des comptes </h1>
...
if(sommeT>0)
{
  ...
  %>
    <tr>
      ...
      <td width="70%"><div align="right"><a href="<%=lien%>?but=finance/listeEntree.jsp&dateDebut=<%=dateDebut%>&dateFin=<%=dateFin%>&idType=<%=typeE[l].getVal()%>&idModePaiement=<%=entite%>&colonne=montant&ordre=DESC"><%=Utilitaire.formaterAr(sommeT)%> Ar </a></div></td>
 </tr>
          <%}}%>
    <tr class="head">
