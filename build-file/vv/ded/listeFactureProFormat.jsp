...
<%!
  String apres="ded/listeFactureProFormat.jsp";
  String lien= null;
  UserEJB u = null;
  TypeObjet dev[] = null;
  Caisse caiss[] = null;
...
<form action="<%=lien%>?but=ded/visualiserFactureProF.jsp" method="post">
...
<%if(pazy!=1){%>
        <a href="<%=lien%>?but=ded/listeLcBcChoix.jsp&pazy=<%=pazy-1%>">&lt;&lt;Page
  ...
  %></td>
    <td width="305" align="right">
      <%
        if(pazy<nbPage){
      %>
      <a href="<%=lien%>?but=ded/listeLcBcChoix.jsp&pazy=<%=pazy%>">Page
      suivante&gt;&gt;</a>
     <%
       }
       %></td>
