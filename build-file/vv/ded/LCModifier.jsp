...
<form action="<%=lien %>?but=ded/LCModifier.jsp&idligne=<%=request.getParameter("idligne")%>&numFact=<%=request.getParameter("numFact")%>&num=<%=request.getParameter("num")%>" method="post" name="listeLC" id="listeLC">
...
<form action="<%=lien%>?but=ded/apresModifLC.jsp" method="POST" name="frmListe" id="frmListe">
  ...
  %>
     ...
     <div align="center">
        ...
        <input type="hidden" name="ordre" value="op">
     </div>
   </form>
   <br />
   <!-- Pagination -->
   <table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
        <tr><td height="25"><b>Nombre de r&eacute;sultat :</b> <%=lcc.length%></td><td align="right"><strong>page</strong> <%=pazy%> <b>sur</b> <%=Math.floor(lcc.length/nbParPazy)+1%></td>
            ...
            <td width="295" valign="top" height="25">
                <%if(pazy>1){%>
                <a href="<%=lien+"?but=ded/LCModifier.jsp"+suivant+"&pazy="+(pazy-1)+"&num="+request.getParameter("num")+"&numFact="+request.getParameter("numFact")%>">&lt;&lt;Page
