	...
	try
	{
		...
		nature=request.getParameter("nature");
          ...
          if (acte.compareTo("INSERT")==0)
          {
			...
			rep=u.createDetailMvtCaisse(request.getParameter("debit"),request.getParameter("credit"), request.getParameter("tiers"),request.getParameter("remarque"),request.getParameter("idMere"));
	
%>
          <script language="JavaScript"> document.location.replace("<%=lien%>?but=ded/detailMvtCaisse.jsp&idMere=<%=request.getParameter("idMere")%>&nature=proformat");
		  </script>
          <%
          }
