...
<%
	...
	pr.setApres("ded/listeOPPrevision.jsp");
...
<form action="<%=pr.getLien()%>?but=ded/listeOPPrevision.jsp" method="post" name="listeop" id="listeop">
	<%
	out.println(pr.getFormu().getHtmlEnsemble());
	%>
	etat : 	<select name="etat" class="champ" id="etat">
				<option value="OPPREVISION">Tous</option>
				<option value="OPPREVISIONAVISER"  <%if(etat.compareTo("OPPREVISIONAVISER")==0) out.println("selected");%>>Non vise</option>
				<option value="OPPREVISIONVISE"  <%if(etat.compareTo("OPPREVISIONVISE")==0) out.println("selected");%>> Vise</option>
			</select>
</form>
<%
