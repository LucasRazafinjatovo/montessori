...
try
{
  ...
  quantite = request.getParameter("quantite"); System.out.println(quantite);
  prixarticle = ArticlePrix.getArticlePrix(id);
  montant = prixarticle * Double.parseDouble(quantite);
  ...
  if(ut.getIdrole().compareTo("dg")==0)
  {
    ...
    out.println("<script language='JavaScript'> document.location.replace('../"+lien+"?but=stock/factureFournisseur.jsp&id="+id+"&montant="+montant+"&designation="+remarque+"&fournisseur='); </script>");
  }
}catch (Exception e){
 %>
