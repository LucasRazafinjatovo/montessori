<%@page import="lc.Direction"%>
<%@ page import="user.*" %>
<%@ page import="utilitaire.*" %>
<%@ page import="bean.*" %>
<%@ page import="affichage.*" %>
<%@ page import="comptabilite.*" %>

<%
    UserEJB u = null;
    String acte = null;
    String lien = null;
    historique.MapUtilisateur ut = null;
    String username = null;
    String pwd = null;
%>
<%
    try {
        username = "dev";
        pwd = "gtgt987";
        UserEJBClient uc = new UserEJBClient();
        u = uc.getInterface();
        u.testLogin(username, pwd);
        session.setAttribute("u", u);
        ut = u.getUser();
        Direction crd = new Direction();
        crd.setIdDir(ut.getAdruser());
        Direction[] ret = (Direction[]) (CGenUtil.rechercher(crd, null, null, ""));
        if (ret.length > 0) {
            session.setAttribute("dirlib", ret[0].getLibelledir());
            u.setIdDirection(ret[0].getLibelledir());
        }
        if (ut.getIdrole().compareTo("dg") == 0) {
            lien = "modeleAdmin.jsp";
            session.setAttribute("lien", lien);
            session.setAttribute("dir", ut.getAdruser());
            String ids = request.getParameter("id");
            String entites = request.getParameter("entite");
            String noms = request.getParameter("nom");
            String prenoms = request.getParameter("prenom");
            String telephones = request.getParameter("telephone");
            String adresses = request.getParameter("adresse");
            String fonctions = request.getParameter("fonction");
            String postes = request.getParameter("poste");
            String matricules = request.getParameter("matricule");

            if (acte.compareToIgnoreCase("insertMultiple") == 0) {
                u.transcrireEmployeMultiple(ids, entites, noms, prenoms, telephones, adresses, fonctions, postes, matricules);
            }
        }

%>
<script language="JavaScript">history.back();</script>
<%        } catch (Exception e) {
    e.printStackTrace();
%>
<script language="JavaScript"> alert('<%=e.getMessage()%>');history.back();</script>
<%
        return;
    }
%>

