...
p.setNomTable("balance_groupe_libelle");
String listeCrt[]={"exercice","compte","intitule","daty","echeance"};
String listeInt[]={"compte","daty", "echeance"};

String colDefaut[]={"exercice", "societe","compte", "intitule"};
...
pr.setApres("comptabilite/balance-etat.jsp");

pr.getFormu().getChamp("compte1").setPageAppel("comptabilite/choixCompte.jsp");
pr.getFormu().getChamp("compte2").setPageAppel("comptabilite/choixCompte.jsp");
...
Balance[] result = (Balance[]) pr.getRs().getResultat();
String daty1 = request.getParameter("daty1");

UserEJB u = (UserEJB) session.getAttribute("u");
Balance[] listeR =  (Balance[]) u.setInitSoldeBalance(result, daty1);
double[] sommes = (double[]) u.calculSommeInit(listeR);
...
%>
<h1>Balance</h1>
<form action="<%=pr.getLien()%>?but=comptabilite/balance-etat.jsp" method="post" name="listeopg" id="listeopg">
<%
...
<td width=7% align="center" valign="top">Compte</td>
<td width=15% align=center valign=top>Intitul&eacute;</td>
...
<td align=center><a href=<%=pr.getLien()%>?but=comptabilite/grandlivre-complet.jsp&exercice=<%=listeR[i].getExercice()%>&compte=<%=listeR[i].getCompte()%>><%=listeR[i].getCompte() %></a></td>
