...
function required(sText, titre)
{
    ...
    if((sText!=null)&&(sText!=''))
        ...
        throw titre + "Ce champ doit être renseigné";
}
...
{
   sText = trim(sText);
   var ret =  titre + "Format de téléphone non valide";
...
function codepostal(txt, titre)
{
   if(txt!="" && ! /[0-9]{5}/.test(txt))
    throw titre + "5 caractères numériques uniquement !";
   else
    return txt;
}
...
function insee(txt, titre)
{
   ...
   if(txt!="" && txt.length > 32)
    throw titre + "Format invalide !\n 32 Caractères alphanumériques uniquement !";
   else
    return txt;
}
...
function frp(txt, titre)
{
   if(txt!="" && ! /[0-9]{15}/.test(txt))
    throw titre + "Format invalide !\n 15 caractères numériques uniquement !";
   else
    return txt;
}
...
function siret(txt, titre)
{
   if(txt!="" && ! /[0-9]{14}/.test(txt))
    throw titre + "Format invalide !\n 14 caractères numériques uniquement !";
   else
    return txt;
}
...
function ncga(txt, titre)
{
   if(txt!="" && ! /[a-zA-Z0-9]{6}/.test(txt))
    throw titre + "Format invalide !\n 6 caractères alphanumériques uniquement !";
   else
    return txt;
}
...
function btq(txt, titre)
{
    ...
    if(txt=="B" || txt=="T" || txt=="Q" || txt=="")
        ...
        throw "Paramètres Télédeclaration : B ou T ou Q seulement";
}

function refeDossier(txt, titre) {
    if (txt != "" && !/[a-zA-Z0-9]{5}/.test(txt))
        throw titre + "5 caractères alphanumériques uniquement!";
    else
        return txt;
}
