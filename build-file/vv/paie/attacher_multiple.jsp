  ...
  if(paiement!=null && paiement.compareToIgnoreCase("")!=0)
  {
    ...
    for(int i=0;i<taille;i++)
    {
      ...
      if(id!=null && id.compareToIgnoreCase("")!=0)
      {
        ...
        String date_saisie = request.getParameter("date_saisie"+i);
        String mois = request.getParameter("mois"+i);
        String  annee = request.getParameter("annee"+i);
        String mode_paiement = request.getParameter("mode_paiement"+i);
        String etablissement = request.getParameter("etablissement"+i);
        ...
        Solde_attache sold = new Solde_attache(ordrepaiement, id, employe, new Double(montant).doubleValue(),daty,new Integer(mois).intValue(),new Integer(annee).intValue(),mode_paiement,etablissement);
        sommeSolde = sommeSolde + sold.getMontant();
        sold.construirePK(con);
        sold.insertToTable(con);
        sendSolde+="&idsolde"+getRealTaille+"="+id;
        getRealTaille++;
      }
    }
    if(sommeSolde!=montantOP)throw new Exception("La somme des montants de la solde n'est pas le meme que le montant de l'OP");
    con.commit();
  }
  %>
          <script type="text/javascript">
          $.ajax({
          url: '<%=DefaultPaieUrl.urlPaie%>/senddataop?taille=<%=getRealTaille%><%=orp%><%=sendSolde%>',
