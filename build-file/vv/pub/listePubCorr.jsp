...
p.setNomTable("pubcompletacorriger");
String listeCrt[]={"id","libClient","idCat","libAgence","designation","libJournal","numParution","pagemisyazy","datyParution","montantVal"};
String listeInt[]={"numParution","pagemisyazy","datyParution","montantVal"};
PageRecherche pr=new PageRecherche(p,request,listeCrt,listeInt,2);
pr.setUtilisateur((user.UserEJB)session.getValue("u"));
pr.setLien((String)session.getValue("lien"));
pr.setApres("pub/listePubCorr.jsp");
...
<form action="<%=pr.getLien()%>?but=pub/listePubCorr.jsp" method="post" name="listePub" id="listePub">
<%
...
<form name="mep" action="<%=pr.getLien() %>?but=pub/apresListe.jsp" method="post">
	...
	<input type="hidden" name="acte" value="corr">
	<table width=100% border=0 align=center cellpadding=3 cellspacing=3 class=monographe>
		<tr class=head>
			...
			<td width=28% align=center valign=top>correction</td>
		</tr>
		<%
		...
		<tr onmouseover=this.style.backgroundColor='#EAEAEA' onmouseout="this.style.backgroundColor=''">
			...
			<td align=center><input type="text" name="page<%=i%>" value=<%=listeP[i].getCorrection() %> ></td>
		</tr>
