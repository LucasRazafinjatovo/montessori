...
<%!
  String apres="listeFactureRChoixPopUp.jsp";
  String nomFichier="listeFactureRChoixPopUp.jsp";
  String lien= null;
  UserEJB u = null;
  TypeObjet dev[] = null;
  Caisse caiss[] = null;
...
<%
  ...
  nomCol=Utilitaire.getNomColonne(new FactureFournisseur());
                ...
                //factF = u.findFactureF("%", "", "", "%", "%","%", "idFactureFournisseur", "ASC", "");
                         ...
                         ResultatEtSomme rs= u.findFactureFournisseurPage("FACTURECLIENTVISEAOR",idFact,  dateDebut,  dateFin, fournisseur, designation, colonne, ordre, apresW,pazy);
...
<script type="text/javascript">
 function getChoix(){
  setTimeout("document.liste.submit()",800);
}
</script>
<link href="style.css" rel="stylesheet" type="text/css">
<h1>Lister les factures Clients</h1>
<p align="center" class="remarque">(choisir les crit&egrave;res de votre recherche
  concernant les recettes)</p>
<form action="<%=apres%>" method="post" name="listeRecette" id="listeRecette">
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
  <tr>
    <td> <p align="center"><strong><u>RECAPITULATION RECHERCHE</u></strong></p>
      ...
      <br>
      <p align="center"><strong><u>LISTE</u></strong></p><form action="apresChoixFactureFPopUp.jsp" name="liste" method="post">
     ...
     <input name="typeObjet" type="hidden" value="<%=Constante.getObjetFactureF()%>"> <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="monographe">
        <tr class="head">
          <td width="55" align="center" valign="top">num</td>

          <td width="59" align="center" valign="top">Date</td>
                   <td width="107" align="right" valign="top"><div align="center">Designation</div></td>
          <td width="121" align="right" valign="top"><div align="center">Fournisseur</div></td>
          <td width="86" align="right" valign="top"><div align="center"><strong>Montant TTC</strong></div></td>

          <td width="57" align="center" valign="top">Montant Tva</td>
                            <td width="69" align="center" valign="top">Choix</td>
  ...
  %>
        <tr onmouseover="this.style.backgroundColor='#EAEAEA'" onmouseout="this.style.backgroundColor=''">
          <td align="center"><%=factF[i].getTuppleID()%></td>
    ...
    <input type="hidden" name="nombre">
                  <td align="center"><input name="indice" type="radio" id="numObjet" value=<%=factF[i].getTuppleID()%> onMouseDown="getChoix()">
<input type="hidden" name="numObjet<%=i%>" value="<%=factF[i].getIdFactureFournisseur()%>">
<input type="hidden" name="montantObjet<%=i%>" value="<%=factF[i].getMontantTTC()%>">
<input type="hidden" name="champReturn" value="<%=request.getParameter("champReturn")%>">
</td>
...
<table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
<tr><td height="25"><b>Nombre de r&eacute;sultat :</b> <%=factF.length%></td><td align="right"><strong>page</strong> <%=pazy%> <b>sur</b><%=nbPage%></td>
</tr>
    <tr>
    <td width="295" valign="top" height="25">
<%if(pazy!=1){%>
        <a href="<%=nomFichier%>&pazy=<%=pazy-1%>&idFact=<%=Utilitaire.remplacePourcentage(idFact)%>&fournisseur=<%=Utilitaire.remplacePourcentage(fournisseur)%>&dateDebut=<%=dateDebut%>&dateFin=<%=dateFin%>&projet=<%=Utilitaire.remplacePourcentage(projet)%>&designation=<%=Utilitaire.remplacePourcentage(designation)%>&colonne=<%=colonne%>&ordre=<%=ordre%>">&lt;&lt;Page
