...
System.out.println("ID OPPPPP "+request.getParameter("avecOp"));
for (int i=0;i<idLcArecup.length;i++)
{
  ...
  if(acte.compareToIgnoreCase("update")==0) u.updateMontantUnionIntra("FACTUREFOURNISSEURLC",numObjet,idLcArecup,montant);
  if(acte.compareToIgnoreCase("delete")==0) u.deleteUnionIntraTable("FACTUREFOURNISSEURLC",idUnion);
  if(acte.compareToIgnoreCase("svalide")==0)
  {
   ...
   System.out.println(Constante.getTableFactureFLC()+"******************"+numObjet);
    u.validerMappageLCM(Constante.getTableFactureFLC(),numObjet);
	//else u.validerMappageLCMavecOp(Constante.getTableFactureFLC(),numObjet,request.getParameter("avecOp"));
}
 %>
<script language="JavaScript"> document.location.replace("<%=lien %>?but=lc/apresChoixListeLC.jsp&numObjet=<%=numObjet%>&avecOp=<%=request.getParameter("avecOp")%>"); </script>
<%
}
catch(Exception e){
  e.printStackTrace();
