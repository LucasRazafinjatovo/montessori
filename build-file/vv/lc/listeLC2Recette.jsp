...
crt.setNomTable("typeentreeliste");
...
compte=(TypeObjet[])CGenUtil.rechercher(crt,null,null," order by val asc");
        ...
        dir = u.findDirection("","","","","");
        lcc=u.findFCCParution(idLC,designation,mois1,mois2,idType,numCompte,"%","%","%",entite,direction,colonne,ordre, annee,paruInf,paruSup,"LIGNECREDITRECETTECOMPLET");
...
<h1>Lister les lignes cr&eacute;dit recette</h1>
<p align="center" class="remarque">(choisir les crit&egrave;res de votre recherche
  concernant les lignes cr&eacute;dit)</p>
<form action="<%=lien %>?but=lc/listeLC2Recette.jsp" method="post" name="listeLC" id="listeLC">
      ...
      <td> <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="monographe">
          ...
          <tr>
            ...
            <td align="left"> <select name="annee" class="champ" id="annee">
                ...
                for(int i=2009;i<=anneeEnCours+3;i++){

                %>
                <option <% if(Integer.parseInt(annee) == i) out.println("selected");%>><%= i %></option>
                <%
                        }
                        %>
              </select>
...
<td width="10%" align="right" bgcolor="#C300FF"><strong>Type LC</strong></td>
          ...
          <td width="20%" align="center">Encaisse</td>
          <td width="20%" align="center">Or vise</td>
 <td width="20%" align="center">Factures</td>
          <td width="20%" align="center">Fact - Or</td>
...
<td width="10%" valign="top"><div align="center"><strong>Type</strong></div></td>
  ...
  %>
        <tr>
          ...
          <td align="right" valign="top"><div align="right">
                         <%
                              chemin = lien.concat("?but=ded/listeOR.jsp&idLC="+lcc[i].getIdLigne());
                %>
          ...
          <td align="right" valign="top"><div align="right">
                         <%
                              chemin = lien.concat("?but=facturefournisseur/critereEntreFactureOr.jsp&idLC="+lcc[i].getIdLigne());
                %>
                      <a href="<%=chemin %>" title="Cliquer ici pour afficher les détails"><%=Utilitaire.formaterAr(lcc[i].getMontantFac())  %></a>
                      </div>
          </td>
          <td align="right" valign="top"><div align="right">
                      <%=Utilitaire.formaterAr(lcc[i].getReste())%></div>

          </td>
          <td align="right" valign="top"><div align="center"><%=lcc[i].getEntite()%></div></td>
          ...
          <td align="right"><div align="center"><a href="<%=lien %>?but=lc/visualiserDetailLCRecette.jsp&numObjet=<%=lcc[i].getIdLigne()%>" title="idLigne =<%=lcc[i].getIdLigne()%> enttite = <%=lcc[i].getEntite() %> annee=<%=lcc[i].getAnnee()%>"><%=lcc[i].getIdLigne()%></a> </div></td>
    ...
    <table border="0" cellpadding="0" cellspacing="0" align="center" width="600">
        <tr><td height="25"><b>Nombre de r&eacute;sultat :</b> <%=lcc.length%></td><td align="right"><strong>page</strong> <%=pazy%> <b>sur</b> <%=Math.floor(lcc.length/nbParPazy)+1%></td>
            ...
            <td width="295" valign="top" height="25">
                <%if(pazy>1){%>
                <a href="<%=lien %>?but=lc/listeLC2Recette.jsp<%=suivant%>&pazy=<%=(pazy-1)%>">&lt;&lt;Page
...
<%
      ...
      <table width="75%" border="0" align="center" cellpadding="0" cellspacing="0" class="monographe">
              ...
              <td>
              <%
                              chemin = lien.concat("?but=finance/listeMvtCaisse.jsp&idLC="+l.getIdLigne());
