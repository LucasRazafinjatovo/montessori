...
acte=request.getParameter("acte");if ((acte==null)||acte.compareTo("")==0) acte="save";
 ...
 u.mapperOrFactureClientLCTableau(numObjet, idLcArecup, "-", montant);
 ...
 %>
<script language="JavaScript"> document.location.replace("<%=lien%>?but=ded/ventilationOrFcLc.jsp&numObjet=<%=numObjet%>&avecOp=<%=request.getParameter("avecOp")%>&typeor=<%=request.getParameter("typeor")%>"); </script>
<%
}
catch(Exception e){
  e.printStackTrace();
